...
class SyncManualIssuedDocument extends Command
{
    ...
    public function handle()
    {
        ...
        do {
            $params = [
                'submissionDateFrom' => Carbon::now()->setTimezone(config('services.einvoice.timezone'))->startOfMonth()->format('Y-m-d') . 'T' . Carbon::now()->setTimezone(config('services.einvoice.timezone'))->startOfMonth()->subDays(10)->format('H:i:s') . 'Z',
        ...
        foreach ($manualIssueDocumentsUuid as $index => $uuid) {
            ...
            $manualIssueDocumentProcessor = new ManualIssueDocumentProcessor($document);
            if ($manualIssueDocumentProcessor->isJson()) {
                $manualIssueDocumentProcessor->processJsonDocument();
            } else {
                $manualIssueDocumentProcessor->processXmlDocument();
            }
        }
    }
}
