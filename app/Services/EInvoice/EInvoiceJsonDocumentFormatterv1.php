...
class EInvoiceJsonDocumentFormatterV1
{
    ...
    public function __construct()
    {
        ...
        $this->round = Company::first()->CO_SM_PCS_AMT;
    }
    ...
    public function generateJsonDocument(EinvoiceHeader $eInvoiceHeader, Collection $eInvoiceDetail, DocumentType $documentType): array
    {
        ...
        $totalExcludingTax = $this->totalNetAmt - $additionalDiscountAmount + $additionalFeeAmount;
        $data = [
                ...
                [
                        ...
                        [
                            ...
                            'TaxSubtotal' => [
                                [
                                    'TaxableAmount' => [
                                        [
                                            '_' => round(floatval($this->totalTaxableAmtPerTaxType), $this->round),
                                            'currencyID' => $currencyCode,
                                        ],
                                    ],
                                    'TaxAmount' => [
                                        [
                                            '_' => round(floatval($this->totalTaxAmtPerTaxType), $this->round),
                                            'currencyID' => $currencyCode,
                                        ],
                                    ],
                                    'TaxCategory' => [
                    ...
                    'LegalMonetaryTotal' => [
                        [
                            'LineExtensionAmount' => [
                                [
                                    '_' => round(floatval($this->totalNetAmt), $this->round),
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                            'TaxExclusiveAmount' => [
                                [
                                    '_' => floatval($totalExcludingTax),
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                            'TaxInclusiveAmount' => [
                                [
                                    '_' => round(floatval($this->totalIncTaxAmt), $this->round) - $additionalDiscountAmount + $additionalFeeAmount,
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                            'AllowanceTotalAmount' => [
                                [
                                    '_' => floatval($additionalDiscountAmount),
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                            'ChargeTotalAmount' => [
                                [
                                    '_' => floatval($additionalFeeAmount),
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                            ...
                            'PayableAmount' => [
                                [
                                    '_' => (round(floatval($this->totalPayableAmt), $this->round) - $additionalDiscountAmount + $additionalFeeAmount + floatval($eInvoiceHeader->EINV_ROUNDING_AMT)) - floatval($eInvoiceHeader->EINV_PREPAYMENT_AMT),
                                    'currencyID' => $currencyCode,
                                ],
                            ],
                        ],
    ...
    private function populateDiscountLines($discounts, $subTotal, &$totalDiscount, $currencyCode)
    {
        ...
        foreach ($discounts as $index => $discount) {
            ...
            $discountAmt = round(floatVal(($currentSubTotal * $discount) / 100), $this->round);

            $currentSubTotal -= $discountAmt;
    ...
    private function populateInvoiceLines(Collection $eInvoiceDetail, EinvoiceHeader $eInvoiceHeader)
    {
        foreach ($eInvoiceDetail as $index => $dt) {
            ...
            $subTotal = floatVal($dt->EINV_UNIT_PRICE) * $quantity;
            $discounts = explode(',', $dt->EINV_DISC_REASON);
            $totalDiscount = 0;

            $discountItems = $this->populateDiscountLines($discounts, $subTotal, $totalDiscount, $eInvoiceHeader->EINV_CURR);
            $allowanceCharge = [];
            $allowanceCharge = $discountItems;
            $allowanceCharge[] = [
                'ChargeIndicator' => [
                    [
                        '_' => true,
                ...
                'MultiplierFactorNumeric' => [
                    [
                        '_' => round($dt->EINV_FEE_RATE, $this->round),
                    ],
                ],
                'Amount' => [
                    [
            ...
            $totalExcludeTaxAmount = $subTotal + floatval($dt->EINV_FEE_AMT) - $totalDiscount;

            $this->totalNetAmt += floatval($totalExcludeTaxAmount);
